...
def main(argv):
    ...
    eprint("\n================= Configuration [" + str(os.getpid()) + "] : =================")
    eprint("Time after which a SYN not yet matched with a connection is interpreted as SYN-SCAN:\n %.1fsec " % DEF_SYN_TIMEOUT )
    eprint("Time to wait before a connection is processed to ensure that the matching SYN iss present:\n %.1fsec" % DEF_CON_WAIT)
