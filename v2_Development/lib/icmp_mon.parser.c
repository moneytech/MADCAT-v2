...
ip_daddr\
    ...
    if (iphdr->ihl > 5) //If Options/Padding present (IP Header bigger than 5*4 = 20Byte) print them as hexadecimal string
    {
        ...
        json_do(false, ", \"ip_options_hex\": \"%s\"", hex_string);
        free(hex_string);
    }
    json_do(false, ", "}"); //close IP Header JSON object
    */
...
ntohs(udphdr->check)\
    ...
    //fprintf(stderr, "\n\ndDEBUG: recv_len: %d, iphdr->ihl*4: %d\n\n", recv_len, iphdr->ihl*4); DEBUG
...
ntohs(tcphdr->urg_ptr));
    ...
    if (tcphdr->doff > 5) //If Options/Padding present (TCP Header longer than 5*4 = 20Byte) print them as hexadecimal string
    {
        ...
        json_do(false, ", \"tcp_options_hex\": \"%s\"", hex_string);
        free(hex_string);
