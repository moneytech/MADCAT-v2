...
int main(int argc, char *argv[])
{
        ...
        if (argc == 2) //read config file
        {
            ...
            filter_exp = malloc(filter_exp_len + 1); filter_exp[0] = 0;
            strncpy(filter_exp, get_config_opt(luaState, "raw_pcap_filter_exp"), filter_exp_len + 1); filter_exp[filter_exp_len] = 0;
        ...
        while (1)
        {
            //Sniff packet
            packet = 0;
            packet = (unsigned char*) pcap_next(handle, &header); //Wait for and grab TCP-SYN (see PCAP_FILTER) (Maybe of maybe not BLOCKING!)
            ...
            payload_sha1_str = print_hex_string(payload_sha1, SHA_DIGEST_LENGTH);  //Do not forget to free!
            //Make HexDump output out of binary packet contents
            payload_hd_str = hex_dump(packet_layer3, (size_exceeded ? max_file_size : packet_len), true); //Do not forget to free!
            payload_str = print_hex_string(packet_layer3, (size_exceeded ? max_file_size : packet_len)); //Do not forget to free!
            ...
            //At this point I want to apologize honestly for the nested ternary operator, it IS bad style. But I always wanted to do this to annoy you all. Sorry.
            json_do(true, "{\
\"origin\": \"MADCAT\", \
\"timestamp\": \"%s\", \
\"proto\": \"%s\", \
